...
function filterInput(input) {
    if (!Array.isArray(input))
        return input;
    const stripSetting = process.env.OPENCODE_MULTI_AUTH_STRIP_ITEM_REFERENCES;
    const stripReferences = stripSetting !== '0' && stripSetting !== 'false';
    if (!stripReferences)
        return input;
    return input.filter((item) => item?.type !== 'item_reference');
}
    ...
    return {
        ...
        config: async (config) => {
            ...
            if (!injectModels)
                return;
            const latestModel = (process.env.OPENCODE_MULTI_AUTH_CODEX_LATEST_MODEL || 'gpt-5.4').trim();
            const mode = normalizeModelInjectionMode(process.env.OPENCODE_MULTI_AUTH_MODEL_INJECTION || pluginConfig.modelInjectionMode);
            try {
        ...
        auth: {
            ...
            async loader(getAuth, provider) {
                ...
                if (accounts.length === 0) {
                    console.log('[multi-auth] No accounts configured. Run: codex-multiauth add <alias>');
                    return {};
                }
                    ...
                    while (attempt < maxAttempts) {
                        ...
                        try {
                            ...
                            if (res.status === 401 || res.status === 403) {
                                ...
                                const authRelated403 = code === 'invalid_token' ||
                                ...
                                if (res.status === 401 || (res.status === 403 && authRelated403)) {
                ...
                },
                {
                    ...
                    authorize: async (inputs) => {
                        ...
                        return {
                            url: flow.url,
                            method: 'auto',
                            instructions: `${flow.instructions} for "${alias}"`,
                            callback: async () => {
                                try {
