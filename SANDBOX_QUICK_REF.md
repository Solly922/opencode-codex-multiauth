...
## 1) One-time setup
```bash
git clone <repo-url> codex-multiauth-sandbox
cd codex-multiauth-sandbox
npm ci
npm run build
```

If the target folder is not empty, clone to a fresh temporary location before running tests.
