import { evaluateAccountHealth, sortAliasesByHealth, type AccountHealth } from './account-order.js'
import { getStoreDiagnostics, loadStore, saveStore, saveStoreUnlocked, updateAccount, withWriteLock } from './store.js'
...
export async function getNextAccount(
  ...
  if (aliases.length === 0) {
    ...
    console.error(
      `[multi-auth] No accounts configured. Run: codex-multiauth add <alias>${extra}`
    )
  ...
  if (forceActive && forceState.forcedAlias) {
    ...
    if (forcedAccount) {
      ...
      if (health.isHealthy) {
        ...
        if (token) {
          store = await withWriteLock(() => {
            const nextStore = loadStore()
            const current = nextStore.accounts[forcedAlias]
            if (!current) return nextStore
            nextStore.accounts[forcedAlias] = {
              ...current,
              usageCount: (current.usageCount || 0) + 1,
              lastUsed: now,
              limitError: undefined
            }
            nextStore.activeAlias = forcedAlias
            nextStore.lastRotation = now
            saveStoreUnlocked(nextStore)
            return nextStore
          })
  ...
  for (const candidate of candidates) {
    ...
    store = await withWriteLock(() => {
      const nextStore = loadStore()
      const current = nextStore.accounts[candidate]
      if (!current) return nextStore
      nextStore.accounts[candidate] = {
        ...current,
        usageCount: (current.usageCount || 0) + 1,
        lastUsed: now,
        limitError: undefined
      }
      nextStore.activeAlias = candidate
      nextStore.lastRotation = now
      if (nextIndex) {
        nextStore.rotationIndex = nextIndex(candidate)
      }
      saveStoreUnlocked(nextStore)
      return nextStore
    })
