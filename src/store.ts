...
const DEFAULT_STORE_FILE = 'codex-multiauth-accounts.json'
...
function getReadableStoreFile(): string {
...
function ensureLockTargetFile(): void {
  const canonicalFile = getStoreFile()
  if (fs.existsSync(canonicalFile)) return

  const readableFile = getReadableStoreFile()
  if (readableFile !== canonicalFile && fs.existsSync(readableFile)) {
    ensureDir()
    fs.copyFileSync(readableFile, canonicalFile)
    fs.chmodSync(canonicalFile, 0o600)
    return
  }

  ensureDir()
  fs.writeFileSync(canonicalFile, JSON.stringify(emptyStore(), null, 2), { mode: 0o600 })
}

function withFileLock<T>(fn: () => T): T {
  ensureLockTargetFile()
  const file = getStoreFile()
...
export function loadStore(): AccountStore {
  ...
  ensureDir()
  const file = getReadableStoreFile()
...
function writeStoreUnlocked(store: AccountStore): void {
  ensureDir()
  if (storeLocked) {
...
export function saveStoreUnlocked(store: AccountStore): void {
